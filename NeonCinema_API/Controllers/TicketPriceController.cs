...
namespace NeonCinema_API.Controllers
{
    ...
    public class TicketPriceController : ControllerBase
    {
        ...
        {
            var result = await _ticketPriceRepository.SetupPrice(request);
        ...
        {
            var result = await _ticketPriceRepository.UpdateTicketPriceAsync(ticketPriceDTO);

            if (result.IsSuccessStatusCode)
            {
                return Ok("Sửa giá vé thành cổn");
            }
